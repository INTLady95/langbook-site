/* Paywall — one-time purchase, the course is yours forever (Tanya's final model:
   ...
   КЛЮЧИ (формат — префикс решает, что открыто; после дефиса — минимум 8 знаков):
     ...
     RUA1-XXXXXXXX … RUC2-XXXXXXXX   — один курс русского (объяснения на английском)
     PLA1-/PLA2-/PLB1-/PLB2-XXXXXXXX — один курс польского (A1…B2)
     ENALL-/ESALL-/RUALL-/PLALL-XXXXXXXX — вся серия языка A1→C2
     ALL-XXXXXXXX                    — мастер-ключ (всё; для Тани/подарков)
   Вступительные курсы (алфавит и чтение): ru-abc / en-abc / es-abc — БЕСПЛАТНЫЕ
   бонусы к A1, открыты всем без ключа (см. freeCourses ниже).
...
window.PAYWALL = {
  ...
  checkout: {                     // courseId → checkout-URL от MoR
    ...
    'ru-a1':'', 'ru-a2':'', 'ru-b1':'', 'ru-b2':'', 'ru-c1':'', 'ru-c2':'',
    'pl-a1':'',
    'pl-a2':'',
    'pl-b1':'',
    'pl-b2':'',
    'pl-c1':'',
    'pl-c2':'',
    'en-series':'', 'es-series':'', 'ru-series':'', 'pl-series':''  },
  freeCourses: ['ru-abc','en-abc','es-abc','pl-abc'],  // бесплатные вступительные курсы (алфавит/чтение) — бонус покупателям A1 и лид-магнит
  buyPage: '../buy.html',
};
...
window.keyCovers = function(key, course){
  ...
  var m = key.match(/^(EN|ES|RU|PL)(A1|A2|B1|B2|C1|C2|ALL)-/);
  if (!m) return false;
...
window.licenseOk = function(course){
  ...
  if ((PAYWALL.freeCourses||[]).indexOf(course) !== -1) return true;  // бесплатный бонус
  var raw = '';
