...
/* voice for Мира's own narration (intros, rule audio). Default Russian, because
   EN/ES courses explain IN Russian. A course that teaches Russian to foreigners
   sets speechNative:'en-US' so the explanations aren't spoken with a Russian voice. */
const SPEECH_NATIVE = LEVEL.speechNative || 'ru-RU';
...
/* i18n of the COURSE PROGRAMME (week/stage titles, day titles + previews, exam
   titles, the final note). The strings are authored in the level-*.js files —
   in Russian for the en/es/pl lines, in English for the Russian-for-foreigners
   line — and stay the fallback. Translations live in courses/i18n-courses.js
   under the key namespace crs.<course>.<field>, e.g.
     crs.en-a1.w2.t   week 2 title
     crs.en-a1.d8.t   day 8 title       crs.en-a1.d8.p   day 8 preview
     crs.en-a1.x3.t   week-3 exam title crs.en-a1.finalNote
   With no dictionary loaded (or a missing key) the authored text shows — the
   course never breaks because of a translation. */
const CRS = LEVEL.course || '';
function ct(key, authored){ return T('crs.' + CRS + '.' + key, authored); }
/* language Мира explains IN — derived from her narration voice, so no level
   file has to be edited; a course may still override with LEVEL.explain */
const EXPLAIN_LANG = (LEVEL.explain || SPEECH_NATIVE || 'ru').slice(0,2);
...
function norm(s){ return (s||'').toLowerCase().replace(/́/g,'').replace(/[.,!?'"’`]/g,'').replace(/\s+/g,' ').trim(); }
...
/* ===== экранная клавиатура для букв целевого языка =====
   Проверка ответа регистр снимает, а диакритику НЕ сворачивает: «zle» не равно «źle».
   Без польской раскладки часть заданий на ввод стала бы непроходимой, поэтому под
   каждым полем ввода стоит ряд букв, которых на обычной клавиатуре нет.
   Ключ — язык курса, так что ряд сам подстраивается под уровень. */
const DIA_KEYS = {
  pl: ['ą','ć','ę','ł','ń','ó','ś','ź','ż'],
  es: ['á','é','í','ó','ú','ü','ñ','¿','¡']
};
const DIA = DIA_KEYS[SPEECH_LANG.slice(0,2)] || [];
/* Ответ на языке объяснений (перевод НА русский) этих букв не требует — там ряд лишний. */
function diaBar(on){ if(!DIA.length || on===false) return '';
  return `<div class="dia-bar" title="${escA(T('eng.diaBarTitle','Нажми, чтобы вставить букву — если её нет на твоей клавиатуре'))}">`
    + `<button type="button" class="dia-shift" aria-label="${escA(T('eng.diaShift','Заглавные'))}">⇧</button>`
    + DIA.map(c=>`<button type="button" class="dia-key" data-c="${c}">${c}</button>`).join('')
    + `</div>`; }
/* Одна делегация на весь документ — работает и в уроке, и в экзамене, и после перерисовки. */
document.addEventListener('click', e => {
  const shift = e.target.closest('.dia-shift');
  if(shift){ const bar=shift.closest('.dia-bar'), up=bar.classList.toggle('up');
    bar.querySelectorAll('.dia-key').forEach(k=>{ const c=k.dataset.c; k.textContent = up?c.toUpperCase():c; });
    shift.classList.toggle('on',up); return; }
  const key = e.target.closest('.dia-key');
  if(!key) return;
  const box = key.closest('.w-type,.w-write,.exq');
  const inp = box && box.querySelector('input[type="text"],textarea');
  if(!inp) return;
  const s = inp.selectionStart==null?inp.value.length:inp.selectionStart;
  const t = inp.selectionEnd==null?s:inp.selectionEnd;
  inp.value = inp.value.slice(0,s) + key.textContent + inp.value.slice(t);
  const p = s + key.textContent.length;
  inp.focus(); try{ inp.setSelectionRange(p,p); }catch(_){}
  /* Поля экзамена слушают input, чтобы снять пометку «неверно» — событие обязательно. */
...
function widget(t,hint){ hint=hint||'';
  ...
  if(t.type==='type') return `${t.q?`<div class="q">${t.q}</div>`:''}<div class="w-type" data-cp data-accept="${escA((t.accept||[]).join('|'))}">${hintChip(hint)}<div class="fillrow"><input type="text" autocomplete="off" spellcheck="false" placeholder="${escA(t.placeholder||'…')}"><button class="btn check">${T('eng.checkBtn','Проверить')}</button></div>${diaBar()}<div class="fb"></div></div>`;
  if(t.type==='speak') return `<div class="w-speak" data-cp data-want="${escA(t.want||'')}">${hintChip(hint)}<div class="speak-row"><button class="mic">🎤</button><div><div class="say-target">“${t.target}”</div><div class="say-sub">${t.sub||''} · <button class="play" data-say="${escA(strip(t.target))}">🔊</button> ${T('eng.sample','образец')}</div></div><button class="btn self">${T('eng.iSaidIt','Я сказал(а)')}</button></div><div class="fb"></div></div>`;
  return '';
}
...
function renderPicture(P){ var visual = P.img
  ...
  return `<div class="block">${P.intro?guide(P.intro,'',true):''}<div class="task">${howto('🖼 <b>'+T('eng.describePicLabel','Опиши картинку.')+'</b> '+(P.prompt||'')+(P.hint?' <span style="color:var(--ink-3)">('+P.hint+')</span>':''))}${visual}<div class="w-write" data-cp><textarea class="write-area" placeholder="${escA(T('eng.describePicPh1','Опиши картинку '))}${LANG_PO}…"></textarea>${diaBar()}<div class="write-tools"><button class="btn write-done">${T('eng.doneBtn','Готово ✓')}</button><button class="btn ghost write-ex">${T('eng.showExample','Показать пример')}</button></div><div class="write-example"><div class="we-label">${T('eng.possibleAnswerLabel','Возможный ответ')}</div>${P.example||''}</div><div class="fb"></div></div></div></div>`; }
function renderDay(d){
  /* the day headline is programme chrome → translated; the echo underneath is
     the theme in the language being taught → stays as authored. Once the
     headline is translated the two can collide (EN title = EN echo) — then the
     echo is dropped rather than printed twice. */
  const dTitle=ct('d'+d.day+'.t',d.themeRu);
  const dEcho=(strip(d.themeEn||'').trim().toLowerCase()===strip(dTitle).trim().toLowerCase())?'':d.themeEn;
  let h=`<div class="dayhead"><div class="eyebrow">${T('eng.week','Неделя')} ${d.week} · ${T('eng.day','День')} ${String(d.day).padStart(2,'0')}</div><h1>${dTitle}</h1>${dEcho?`<div class="echo">${dEcho}</div>`:''}</div>`;
...
/* Owner QA: an explicit owner unlock (localStorage 'langbook-owner', set by visiting
   any page with ?unlock=1) makes every week browsable — no exam-gated progression.
   Off by default and in tests (which clear localStorage), so the progression gate behaves normally. */
var LOCAL_QA = (function(){ try { return localStorage.getItem('langbook-owner') === '1'; } catch(e){ return false; } })();
...
function weekUnlocked(w){ return LOCAL_QA || w===1 || examPassed(w-1) || (LEVEL.preview && COURSE[w-1] && COURSE[w-1].days.some(d=>!!DAYS[d.n])); /* LOCAL_QA: owner browses all weeks locally; preview levels: a built week is browsable before the exam */ }
...
function renderMap(){
  ...
  COURSE.forEach((w,i)=>{
    ...
    html+=`<div class="week${unlocked?'':' locked'}"><div class="week-head"><span class="wn">${T('eng.week','Неделя')} ${w.wn}</span><span class="wt">${ct('w'+wk+'.t',w.wt)}</span>${unlocked?(passed?'<span class="wlock done">✓ '+T('eng.weekPassed','неделя сдана')+'</span>':''):`<span class="wlock">${T('eng.examGate1','🔒 сдай экзамен за Неделю ')}${String(wk-1).padStart(2,'0')}</span>`}</div><div class="days">`;
    html+=w.days.map(d=>{
      const live=!!DAYS[d.n];
      if(!unlocked) return `<div class="day locked" data-locked="${wk}"><div class="dn">${String(d.n).padStart(2,'0')}<small>${T('eng.dayShort','день')}</small></div><div class="di"><div class="dt">${ct("d"+d.n+".t",d.t)}</div><div class="dp">${ct("d"+d.n+".p",d.p)}</div></div><span class="badge">🔒</span></div>`;
      const done=live&&dayDone(d.n);
      const paid=live&&window.PAYWALL&&PAYWALL.enabled&&!licenseOk()&&d.n>PAYWALL.freeDays;
      if(paid) return `<div class="day paid" data-buy="1"><div class="dn">${String(d.n).padStart(2,'0')}<small>${T('eng.dayShort','день')}</small></div><div class="di"><div class="dt">${ct("d"+d.n+".t",d.t)}</div><div class="dp">${ct("d"+d.n+".p",d.p)}</div></div><span class="badge">🔒 ${T('eng.paidBadge','полная версия')}</span></div>`;
      return `<${live?'a':'div'} class="day ${live?'live':'soon'}${done?' done':''}"${live?' data-day="'+d.n+'" href="#"':''}><div class="dn">${String(d.n).padStart(2,'0')}<small>${T('eng.dayShort','день')}</small></div><div class="di"><div class="dt">${ct("d"+d.n+".t",d.t)}</div><div class="dp">${ct("d"+d.n+".p",d.p)}</div></div><span class="badge">${done?'✓ '+T('eng.done','пройдено'):(live?T('eng.start','начать'):T('eng.soon','скоро'))}</span></${live?'a':'div'}>`;
    }).join('');
    if(ex){
...
function renderExam(ex){
  ...
  h+=`<div class="dayhead exam-head"><div class="eyebrow">${ex.final?T('eng.finalExamTitle','Финальный экзамен'):T('eng.checkpointWeekPrefix','Чекпоинт · Неделя ')+String(ex.week).padStart(2,'0')}</div><h1>${ct("x"+ex.week+".t",ex.title)}</h1><div class="echo">${ex.final?'Final test':'Weekly checkpoint'}</div></div>`;
...
function exqBody(q,t){
  ...
  /* trER — перевод НА язык объяснений: буквы целевого языка там не нужны, ряд не показываем. */
  if(t==='gap'||t==='trRE'||t==='trER'){ const ph=t==='trER'?T('eng.phRussian','по-русски…'):(t==='gap'?T('eng.phFillWord','впиши слово…'):PH_IN); return `<input class="exq-in" type="text" autocomplete="off" autocapitalize="off" spellcheck="false" placeholder="${escA(ph)}">${diaBar(t!=='trER')}`; }
  ...
  if(t==='describe'){ const fig=q.img?`<figure class="exq-figure"><img src="${q.img}" alt="${escA(T('eng.pictureAlt','Картинка для описания'))}" loading="lazy"></figure>`:''; return `${fig}<textarea class="exq-ta" placeholder="${q.img?escA(T('eng.describePicPh1','Опиши картинку ')+LANG_PO+'…'):escA(T('eng.writeAnswerPh1','Напиши ответ ')+LANG_PO+'…')}"></textarea>${diaBar()}<div class="exq-wc"></div>`; }
  return '';
}
...
function gradeExam(root,w){
  ...
  if(passed){
    ...
    res.innerHTML = ex.final
      ? `<div class="ex-seal">🏆</div><h3>${pct}${T('eng.finalPassHead1','% — ты прошёл(ла) весь ')}${LEVEL.code}!</h3><p>${ct("finalNote",LEVEL.finalNote)}</p><button class="btn ex-done">${T('eng.backToProgram','← К программе курса')}</button>`
...
function wireExqMatch(qEl,item,onChange){
  ...
  shuffle(item.pairs.slice()).forEach(p=>{ const tl=document.createElement('div'); tl.className='exq-mtile'+([...String(p[0])].length<=2?' emoji':''); tl.textContent=p[0]; tl.draggable=true; tl.dataset.w=p[1];
...
function exNorm(s){ return (s||'').toLowerCase().replace(/́/g,'').replace(/[.,!?;:'"’`«»()]/g,'').replace(/ё/g,'е').replace(/\s+/g,' ').trim(); }
...
function dayName(n){ for(const w of COURSE) for(const d of w.days) if(d.n===n) return ct("d"+n+".t",d.t); return ''; }
...
function wireDay(root){
  ...
  root.querySelectorAll('.listen[data-ru]').forEach(b=>b.addEventListener('click',()=>speak(b.dataset.ru,{lang:SPEECH_NATIVE,rate:.98})));
  ...
  root.querySelectorAll('.w-speak').forEach(w=>{ const mic=w.querySelector('.mic'),self=w.querySelector('.self'),f=w.querySelector('.fb'),want=(w.dataset.want||'').toLowerCase(); const SR=window.SpeechRecognition||window.webkitSpeechRecognition;
    // «Я сказал(а)» is the CHECK button: the mic records (no verdict yet), then this
    // button digests what was heard and flips to a green «Готово ✓» done state.
    function markDone(said){ w.dataset.done='1'; self.classList.remove('checking','ready'); self.classList.add('done'); self.innerHTML=T('eng.speakDone','Готово ✓'); fb(f,true, said?('<b>'+T('eng.heardLabel','Услышала:')+'</b> «'+said+T('eng.heardGoodSuffix','». Отлично сказано! 🎤')):T('eng.speakSelfDone','<b>Засчитано!</b> Проговаривай вслух почаще — так речь закрепляется быстрее всего.')); tick(w,true); }
    mic.addEventListener('click',()=>{ if(w.dataset.done)return; if(!SR){ fb(f,true,T('eng.speechUnavailable','Распознавание недоступно в этом браузере — просто скажи фразу вслух и нажми «Я сказал(а)».')); return; }
      const r=new SR(); r.lang=SPEECH_LANG; r.interimResults=true; r.maxAlternatives=1; let fin='',heard=false,errored=false;
      mic.classList.add('rec'); fb(f,true,T('eng.listeningPrompt','Слушаю… говори чётко и не спеша.'));
      const guard=setTimeout(()=>{ try{r.stop();}catch(e){} },8000); // не виснем, если микрофон занят другим приложением (напр. Notion)
      r.onresult=ev=>{ heard=true; let interim=''; for(let i=ev.resultIndex;i<ev.results.length;i++){ const res=ev.results[i]; if(res.isFinal) fin+=res[0].transcript; else interim+=res[0].transcript; } const live=(fin+' '+interim).toLowerCase().trim(); if(live&&!fin) fb(f,true,'<b>'+T('eng.heardLabel','Услышала:')+'</b> «'+live+'…»'); };
      r.onerror=e=>{ errored=true; mic.classList.remove('rec'); clearTimeout(guard); const busy=e&&(e.error==='not-allowed'||e.error==='audio-capture'||e.error==='service-not-allowed'||e.error==='aborted'); fb(f,true, busy?T('eng.micBusy','Похоже, микрофон занят другим приложением (например, Notion). Закрой его — или просто скажи вслух и нажми «Я сказал(а)».'):T('eng.noSoundCaught','Не поймала звук — скажи вслух и нажми «Я сказал(а)».')); };
      r.onend=()=>{ mic.classList.remove('rec'); clearTimeout(guard); const said=fin.toLowerCase().trim(); if(!said){ if(!heard&&!errored) fb(f,true,T('eng.noSoundCaught','Не поймала звук — скажи вслух и нажми «Я сказал(а)».')); return; } w.dataset.heard=said; self.classList.add('ready'); fb(f,true,'<b>'+T('eng.heardLabel','Услышала:')+'</b> «'+said+T('eng.heardCheckSuffix','». Нажми «Я сказал(а)», чтобы проверить.')); };
      try{ r.start(); }catch(e){ mic.classList.remove('rec'); clearTimeout(guard); fb(f,true,T('eng.micBusy','Похоже, микрофон занят другим приложением (например, Notion). Закрой его — или просто скажи вслух и нажми «Я сказал(а)».')); }
    });
    self.addEventListener('click',()=>{ if(w.dataset.done||self.classList.contains('checking'))return; self.classList.remove('ready'); self.classList.add('checking'); self.innerHTML=T('eng.checking','Проверяю…'); fb(f,true,T('eng.checkingMsg','Проверяю, что ты сказал(а)…'));
      setTimeout(()=>{ const said=(w.dataset.heard||'').trim();
        if(!want||!said||said.includes(want)||said.length>2){ markDone(said); }
        else { self.classList.remove('checking'); self.innerHTML=T('eng.iSaidIt','Я сказал(а)'); fb(f,false,'<b>'+T('eng.heardLabel','Услышала:')+'</b> «'+said+T('eng.heardTryAgainSuffix','». Почти — попробуй ещё раз, чуть чётче.')); }
      },700);
    });
  });
}
...
/* ===== "the lessons are explained in …" notice =====
   The UI language and the language Мира explains in are two different things:
   the storefront can be read in EN/ES while a course is still taught in
   Russian. When they differ, say so quietly under the hero lead. */
(function(){
  var ui = window.lbLang || 'ru';
  if (ui === EXPLAIN_LANG) return;                       // nothing to warn about
  var NOTE = { ru:T('eng.explainedIn.ru','Уроки этого курса объясняются по-русски.'),
               en:T('eng.explainedIn.en','Уроки этого курса объясняются по-английски.'),
               es:T('eng.explainedIn.es','Уроки этого курса объясняются по-испански.') };
  var txt = NOTE[EXPLAIN_LANG]; if (!txt) return;
  var lead = document.querySelector('.hero .lead'); if (!lead) return;
  var p = document.createElement('p');
  p.className = 'explang'; p.textContent = txt;
