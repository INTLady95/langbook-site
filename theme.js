...
(function(){
  /* Owner unlock: visiting any page with ?unlock=1 stores a master license key
     + an owner flag in THIS browser (the real paywall mechanism), then reloads
     to a clean URL so every lesson and week is open. Nothing is bypassed in code,
     so tests (which clear localStorage) and real visitors are unaffected. */
  try {
    if (/[?&]unlock=1(?:&|$)/.test(location.search)) {
      localStorage.setItem('langbook-owner', '1');
      var _lic = localStorage.getItem('langbook-license') || '';
      if (_lic.indexOf('ALL-') === -1) localStorage.setItem('langbook-license', (_lic ? _lic + ' ' : '') + 'ALL-OWNER0000');
  ...
  function buildSidebar(home){
    ...
    d.innerHTML = ''
      ...
      + '</nav>'
      /* language + theme live in the top site-bar (always visible, mobile too);
         no duplicate here — the drawer is nav / settings / contact / notes only. */
      + '<div class="lb-d-foot">'+tagline()+'<br><span>'+BRAND.name+' · '+BRAND.version+'</span></div>';
